...
model.load_weights('model_weights_scaler_weekends.h5')
...
last_weekend = (df['time'][0].weekday() * 24 + valid_last % y_len) % len(weekend_seq)
...
for i in range(number_of_predictions):
    if not i:
        ...
        weekends, last_weekend = gen_next_seq(weekend_seq, last_weekend, y_len)
...
with open('model_weights_scaler_weekends_preds.json', 'w') as file:
